...
export default function ThinkHub() {
  ...
  const [debateMessages, setDebateMessages] = React.useState([]);
  ...
  const startDebate = (topic) => {
    ...
    setDebateMessages([
      ...
      {
        type: "ai",
        content: topic.aiInitialResponse,
        timestamp: new Date()
      }
    ]);
    setUserInput("");
  ...
  const sendUserMessage = async () => {
    if (!userInput.trim() || !debateData) return;
        ...
        <div className="mt-6 rounded-2xl border border-slate-200 bg-white/80 p-6 shadow-lg">
          ...
          <div className="space-y-4 max-h-[60vh] overflow-y-auto mb-6 pr-2">
            ...
            {aiThinking && (
              ...
              >
                <div className="bg-gradient-to-br from-slate-100 to-slate-50 border border-slate-200 rounded-2xl p-4">
                  <div className="flex items-center gap-2">
                    <span className="text-xl">🤖</span>
                    <span className="text-xs font-semibold opacity-70">ЖИ</span>
                    <span className="ml-2 text-xs text-slate-500 animate-pulse">Ойлап жатыр...</span>
                  </div>
                </div>
              </motion.div>
            )}
          ...
          <div className="border-t border-slate-200 pt-4">
            <div className="flex gap-3">
              <textarea
                ...
                placeholder="Сіздің пікіріңізді жазыңыз..."
                className="flex-1 rounded-xl border-2 border-slate-300 px-4 py-3 text-sm focus:outline-none focus:border-[#1F7A8C] focus:ring-2 focus:ring-[#1F7A8C]/20 resize-none"
                rows={3}
              />
              <button
                ...
                className={`px-6 py-3 rounded-xl font-semibold text-sm transition-all ${
                  userInput.trim() && !aiThinking
                    ? "bg-gradient-to-r from-[#1F7A8C] via-[#1aa6b5] to-[#0ea5a5] text-white hover:shadow-lg hover:scale-105"
