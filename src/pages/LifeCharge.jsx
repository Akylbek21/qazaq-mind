...
export default function LifeCharge() {
  ...
  const setFocusMinutes = (min) =>
    setSt((p) => ({ ...p, focusMinutes: min, ...(p.timerMode === "focus" && !p.timerRunning ? { timerLeftSec: min * 60 } : {}) }));
  const setBreakMinutes = (min) =>
    setSt((p) => ({ ...p, breakMinutes: min, ...(p.timerMode === "break" && !p.timerRunning ? { timerLeftSec: min * 60 } : {}) }));
  const startPause = () => setSt((p) => ({ ...p, timerRunning: !p.timerRunning }));
  const resetTimer = () => setSt((p) => ({ ...p, timerRunning: false, timerLeftSec: (p.timerMode === "focus" ? p.focusMinutes : p.breakMinutes) * 60 }));
  const switchMode = (mode) => setSt((p) => ({ ...p, timerMode: mode, timerRunning: false, timerLeftSec: (mode === "focus" ? p.focusMinutes : p.breakMinutes) * 60 }));
  const toggleChecklistLocal = (key) => setSt((p) => ({ ...p, checklist: { ...p.checklist, [key]: !p.checklist[key] } }));
  ...
  const loadPq = async () => {
    setPqLoading(true); setPqErr("");
    try {
      const list = await fetchPQTasks();           // сервер → немесе фоллбэк тізім
      setPqTasks(Array.isArray(list) ? list : []);
      const init = {}; list.forEach((t) => (init[t.id] = !!init[t.id]));
      setPqDoneMap(init);
    } catch (e) {
      setPqErr(humanize(e));
      setPqTasks([]); setPqDoneMap({});
    } finally {
      setPqLoading(false);
    }
  };
  React.useEffect(() => { loadPq(); }, []);

  const togglePq = async (taskId) => {
    setPqDoneMap((m) => ({ ...m, [taskId]: !m[taskId] })); // optimistic
    try {
      const completed = await togglePQTask(taskId, todayStr());
      setPqDoneMap((m) => ({ ...m, [taskId]: !!completed }));
    } catch (e) {
      setPqDoneMap((m) => ({ ...m, [taskId]: !m[taskId] })); // rollback
      setPqErr(humanize(e));
    }
  };
  ...
  return (
    <div className="container mx-auto max-w-6xl px-4 py-10">
      ...
      <p className="mt-2 text-slate-600">Фокус-таймер, су трекері, ұйқы/DND және серверлік Detox-чеклист. Дерек құрылғыда сақталады.</p>
      ...
      <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="rounded-2xl border border-emerald-200 bg-white p-5 shadow-sm">
          ...
          <div className="mt-4 flex items-center gap-4">
            <button onClick={() => switchMode("focus")} className={`px-3 py-2 rounded-xl text-sm font-semibold border ${st.timerMode === "focus" ? "bg-emerald-600 text-white border-emerald-600" : "border-slate-300"}`}>Focus</button>
            <button onClick={() => switchMode("break")} className={`px-3 py-2 rounded-xl text-sm font-semibold border ${st.timerMode === "break" ? "bg-emerald-600 text-white border-emerald-600" : "border-slate-300"}`}>Break</button>
          ...
          <div className="mt-6 text-center">
            ...
            <div className="mt-4 flex justify-center gap-3">
              <button onClick={startPause} className="px-4 py-2 rounded-xl bg-emerald-600 text-white font-semibold">{st.timerRunning ? "Пауза" : "Бастау"}</button>
              <button onClick={resetTimer} className="px-4 py-2 rounded-xl border border-slate-300 font-semibold">Қалпына келтіру</button>
            </div>
          </div>
          ...
          <div className="mt-6 grid grid-cols-2 gap-3">
            <div>
              ...
              <select value={st.focusMinutes} onChange={(e) => setFocusMinutes(Number(e.target.value))} className="mt-1 w-full rounded-xl border border-slate-300 px-3 py-2">
                {[20, 25, 30, 45, 60].map((m) => (<option key={m} value={m}>{m}</option>))}
              </select>
            </div>
            <div>
              <label className="text-sm text-slate-600">Break уақыты (мин)</label>
              <select value={st.breakMinutes} onChange={(e) => setBreakMinutes(Number(e.target.value))} className="mt-1 w-full rounded-xl border border-slate-300 px-3 py-2">
        ...
        <div className="rounded-2xl border border-sky-200 bg-white p-5 shadow-sm">
          ...
          <div className="mt-4 flex items-center gap-4">
            <button onClick={() => setField({ waterCups: Math.max(0, st.waterCups - 1) })} className="px-3 py-2 rounded-xl border border-slate-300 font-semibold">− 1 стақан</button>
            <div className="text-4xl font-bold tabular-nums">{st.waterCups}</div>
            <button onClick={() => setField({ waterCups: Math.min(99, st.waterCups + 1) })} className="px-3 py-2 rounded-xl bg-sky-600 text-white font-semibold">+ 1 стақан</button>
          ...
          <div className="mt-2 text-sm text-slate-600">Прогресс: {Math.min(100, Math.round((st.waterCups / Math.max(1, st.waterTarget)) * 100))}%</div>
        </div>
        ...
        <div className="rounded-2xl border border-indigo-200 bg-white p-5 shadow-sm">
          ...
          <div className="mt-4 grid grid-cols-2 gap-4">
            <div>
              ...
              <div className="mt-1 text-xs text-slate-500">Ұйқыға дейін: <b>{Math.floor(minutesUntil(st.bedtime) / 60)} сағ {minutesUntil(st.bedtime) % 60} мин</b></div>
            </div>
            <div>
              ...
              <div className="mt-1 text-xs text-slate-500">Оянуға: <b>{Math.floor(minutesUntil(st.wakeTime) / 60)} сағ {minutesUntil(st.wakeTime) % 60} мин</b></div>
            </div>
          </div>
          ...
          <div className="mt-3 text-xs text-slate-500">Ескерту: Бұл тек жоспар/ескерту құралы. Құралдың жүйелік параметрлерін өзгертпейді.</div>
        </div>
        ...
        <div className="rounded-2xl border border-amber-200 bg-white p-5 shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <button onClick={loadPq} className="ml-auto rounded-xl border border-slate-300 px-3 py-1 text-sm font-semibold">Қайта жүктеу</button>
          </div>
          <p className="mt-1 text-sm text-slate-600">Тізім бекэндтен келеді. Белгі қою — бірден серверге жазылады (күн: {todayStr()}).</p>
          ...
          {!pqLoading && pqTasks.length > 0 ? (
            <>
              <div className="mt-4 space-y-3">
                {pqTasks.map((t) => (
                  <label key={t.id} className="flex items-center gap-3">
                    <input type="checkbox" checked={!!pqDoneMap[t.id]} onChange={() => togglePq(t.id)} />
              ...
              <div className="mt-4 text-sm text-slate-600">
                Орындағандар: <b>{Object.values(pqDoneMap).filter(Boolean).length} / {pqTasks.length}</b>
              </div>
            </>
          ) : (
            <>
              <div className="mt-4 text-xs text-slate-500">Сервер қолжетімсіз — локалдық чеклист:</div>
              <div className="mt-2 space-y-3">
                {[
                  ...
                  <label key={key} className="flex items-center gap-3">
                    <input type="checkbox" checked={!!st.checklist[key]} onChange={() => toggleChecklistLocal(key)} />
              ...
              <button
                onClick={() => setSt((p) => ({ ...p, checklist: { morningNoPhone: false, eveningNoPhone: false, walk: false, stretch: false } }))}
